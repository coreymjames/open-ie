...
function ProjectList () {
  ...
  return (
    <div>
      {projects.sort((a,b) => rewardFunction(a, weights) - rewardFunction(b, weights)).map((project, index) => <ProjectCard key={project.id} project={project} index={index + 1} />)}
    </div>
  )
}

function ProjectCard ({project, index}: {project: ProjectWithMetrics, index: number}) {
  return (
